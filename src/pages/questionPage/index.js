            ...
            <div className="container-box">
              ...
              <h5 className="brand-icon"><li></li>BRAND</h5>
                <h3>Developer quiz</h3>
                  <button className="save-progress" onClick={()=> SaveProgress(quesRef[0].number)}>Save Progress</button>
              </div>
              <div className="container-box-padding">
                ...
                {quesRef[0].answers.length && quesRef[0].answers.map((a, index) => (<div className="quzioption">
                    <button className={selected === (index + 1)  ? 'pinkBackground' :   quesRef[0].selected === (index + 1 ) ? 'pinkBackground' : null} onClick={() => handleClick(index,quesRef[0])}>{index + 1}. {' '}  {a}</button>
                </div>))}
                {
                    quesRef[0].number !== 7 ?
                        <>
                        <div className="btn-for-test">
                            ...
                            <button className="previous-btn" disabled={(quesRef[0].number === 1) || parameters.no <= store.save_progress} onClick={() => {
